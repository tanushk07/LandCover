...
def get_available_models():
    ...
    available_configs = [
        f.stem.replace("_config", "") for f in config_dir.glob("*_config.yaml")
    ]
...
def compare_models(models_to_compare, test_only=False, save_results=True):
        ...
        for i, model_name in enumerate(models_to_compare, 1):
            try:
                ...
                matching_models = [
                    ...
                    in f.name.lower().replace("-", "_").replace(" ", "")
                ]

                if matching_models:
                    print(
                        f"⏭️  {model_name.upper()} skipped — found trained model: {matching_models[0].name}"
                    )
                    ...
                    print(
                        f"🔨 Training {model_name.upper()} (no trained model found)..."
                    )
                    ...
                    print(
                        f"✅ {model_name.upper()} completed - IoU: {max_score:.4f}, Time: {results['training_results'][model_name]['training_time_formatted']}"
                    )
    ...
    for i, model_name in enumerate(models_to_compare, 1):
        try:
            ...
            print(
                f"✅ {model_name.upper()} tested - mIoU: {test_metrics.get('mean_iou', 0):.4f}"
            )

        except Exception as e:
            print(f"❌ {model_name.upper()} testing failed: {str(e)}")
            results["test_results"][model_name] = {"error": str(e), "status": "failed"}
    ...
    successful_models = [
        ...
        if "error" not in results["test_results"].get(m, {})
    ]
    ...
    if successful_models:
        ...
        for model_name in successful_models:
            ...
            for metric_name, value in metrics.items():
                if value < -1 or value > 1:
                    print(
                        f"⚠️  WARNING: {model_name} has invalid {metric_name}: {value}"
                    )
            ...
            comparison_data.append(
                {
                    ...
                    **metrics,
                    ...
                    "Training Time (min)": (train_res.get("training_time_seconds") or 0)
                    / 60,
            ...
            print(
                f"✅ {model_name.upper()} added to comparison (mIoU: {metrics['Mean IoU']:.4f})"
            )
            ...
            for metric_key, weight in METRIC_WEIGHTS.items():
                ...
                if col_name in row and pd.notna(row[col_name]):
                    ...
                    if -1 <= value <= 1:  # Basic sanity check
                        ...
                        print(
                            f"⚠️  {col_name} has invalid value {value} for {row.get('Model', 'unknown')}"
                        )
        ...
        results["comparison_summary"] = {
            ...
            "full_ranking": df.sort_values("Weighted Score", ascending=False).to_dict(
                "records"
            ),
        }
        ...
        if save_results:
            ...
            plt.title(
                "Final Weighted Scores (Overall Model Performance)",
                fontsize=14,
                fontweight="bold",
            )
            ...
            plt.tight_layout()
            plt.savefig(
                f"output/model_comparison/weighted_scores_{results['experiment_info']['timestamp']}.png",
                dpi=300,
            )
            plt.close()
...
def create_comparison_plots(df, timestamp):
    ...
    plt.tight_layout()
    plt.savefig(
        output_dir / f"performance_comparison_all_metrics_{timestamp}.png", dpi=300
    )
    plt.close()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compare different segmentation model architectures"
    )
    parser.add_argument(
        "--models",
        nargs="+",
        default=["unet", "deeplabv3", "linknet"],
    ...
    if results["comparison_summary"]:
        ...
        best_score = results["comparison_summary"]["best_model_overall"][
            "weighted_score"
        ]
        print(
